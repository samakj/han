...
class ReportMetricStore:
    ...
    def get_report_metrics(
        ...
        abbreviation: Optional[Union[Set[str], str]] = None,
        ...
        if report_value_type:
            where_conditions.add(
                f"report_value_type = "
                f"{'ANY(:report_value_type)' if isinstance(report_value_type, set) else ':report_value_type'}"
            )
        if unit:
            where_conditions.add(
                f"unit = "
                f"{'ANY(:unit)' if isinstance(unit, set) else ':unit'}"
            )
